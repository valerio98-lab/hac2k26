...
def _proprio_actor_obs() -> dict[str, ObservationTermCfg]:
    return {
        ...
        # Explicit tracking errors — computed from the *ground-truth* EE state
        # inside the observation function, so they would bypass the noise on
        # ``ee_pos_w`` / ``ee_lin_vel_w`` / ``ee_rot6d_w`` / ``ee_ang_vel_w``
        # if left clean. Same magnitude of noise is injected here so the actor
        # cannot dodge sensor noise by reading the (otherwise clean) error
        # channel — proper sim-to-real discipline. The critic still sees clean
        # errors (privileged).
