...
from hac2k26.ee_tracking.config.go1.env_cfgs import unitree_go1_getup_env_cfg
...
def test_t1_friction_dr(t1_env: ManagerBasedRlEnv) -> None:
    ...
    _assert_varied_across_worlds(
        foot_friction[:, :, 1], "T1 foot spin friction (axis 1)"
    )
    _assert_varied_across_worlds(
        foot_friction[:, :, 2], "T1 foot roll friction (axis 2)"
    )
